...
import { Card, Table, Row, Col, Typography, Collapse, Empty, theme } from 'antd';
...
const Dashboard: React.FC = () => {
  ...
  const handleEditClick = (record: TableItem, tableType: 'bonusManagementDrafts' | 'bonusManagementPending' | 'limitsDrafts' | 'limitsPending') => {
    switch (tableType) {
  ...
  const getColumns = (tableType: 'bonusManagementDrafts' | 'bonusManagementPending' | 'limitsDrafts' | 'limitsPending', showEdit: boolean = true) => [
    {
      title: 'Request ID',
      dataIndex: 'requestId',
      key: 'requestId',
        ...
        <span>
          ...
          {record.status === 'Draft' && (
            <a 
              ...
              style={{ cursor: 'pointer', marginLeft: '10px', color: token.colorError }}
            >
              <DeleteOutlined />
            </a>
          )}
  ...
  const renderSection = (title: string, draftsData: TableItem[], pendingData: TableItem[]) => (
        ...
        <Col span={12}>
          <Card title="Drafts" bordered={false}>
            {draftsData.length === 0 ? (
              ...
              <Table
                {...tableProps}
                columns={getColumns(title.toLowerCase().replace(' ', '') + 'Drafts' as any)}
        ...
        <Col span={12}>
          <Card title="Pending" bordered={false}>
            {pendingData.length === 0 ? (
              ...
              <Table
                {...tableProps}
                columns={getColumns(title.toLowerCase().replace(' ', '') + 'Pending' as any)}
